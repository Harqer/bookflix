...
export default function SubmitScreen() {
  ...
  const submitAction = useAction(api.submission.submitBookProtected);
  const ingestAction = useAction(api.ingest.ingestVideo);
  const [isPending, setIsPending] = useState(false);
  const [isUploading, setIsUploading] = useState(false);

  const handleFileUpload = async () => {
    // In a real app, we'd use expo-document-picker here
    setIsUploading(true);
    try {
      // Mocking a file for demonstration as per instructions
      const result = await ingestAction({
        fileName: "book_draft.mp4",
        fileType: "video/mp4",
        fileBase64: "...", // Base64 content from picker
      });
      
      Alert.alert("Upload Successful", "Video saved to AWS S3 Cinematic Storage.");
    } catch (err: any) {
                ...
                {isUploading ? (
                  ...
                  <>
                    ...
                    <Text style={{ color: colors.foreground, fontSize: 15, fontWeight: "700" }}>
                      Upload Source Material (MP4/PDF)
                    </Text>
                  </>
                )}
              </TouchableOpacity>
